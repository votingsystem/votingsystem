...
public class DocumentValidator {
    ...
    private static Logger log = Logger.getLogger(DocumentValidator.class.getName());

    //{"operation":"SEND_VOTE","optionSelectedId":2,"UUID":"cfbeec4a-f87c-4e4f-b442-4b127259fbd5",
    //"optionSelectedContent":"option A","eventURL":"https://192.168.1.5/AccessControl/eventElection/1"}
    public static ResponseVS<Long> validateVote(File voteFile, Set<TrustAnchor> trustAnchors,
        ...
        Set<User> signersVS = signedFile.getSigners();
        ...
        for(User signerVS:signersVS) {
            try {
                if(signerVS.getTimeStampToken() != null) {//user signature
                    CertUtils.verifyCertificate(eventTrustedAnchors, false, Arrays.asList(signerVS.getX509Certificate()),
                            voteDate);
                } else {//server signature
                    CertUtils.verifyCertificate(trustAnchors, false, Arrays.asList(signerVS.getX509Certificate()), voteDate);
                }
            } catch(Exception ex) {
                log.log(Level.SEVERE, ex.getMessage(), ex);
    ...
    public static ResponseVS validateRepresentationDocument(File signedFile, Set<TrustAnchor> trustAnchors, Date dateBegin,
        ...
        try {
            CertUtils.verifyCertificate(trustAnchors, false, Arrays.asList(user.getX509Certificate()));
            ...
            return new ResponseVS(ResponseVS.SC_ERROR, ContextVS.getInstance().getMessage("certificateErrorMsg",
                    user.getNif(), signedFile.getName()));
        }
        try {
            SignerInformationVerifier timeStampSignerInfoVerifier = new
                    JcaSimpleSignerInfoVerifierBuilder().build(timeStampServerCert);
            user.getTimeStampToken().validate(timeStampSignerInfoVerifier);
        } catch(Exception ex) {
            log.log(Level.SEVERE, ex.getMessage(), ex);
            return new ResponseVS(ResponseVS.SC_ERROR, ContextVS.getInstance().getMessage("timestampValidationErrorMsg",
                    signedFile.getName()));
        }
        Date tokenDate = user.getTimeStampToken().getTimeStampInfo().getGenTime();
        ...
        Map dataMap = cmsMessage.getSignedContent(new TypeReference<Map<String, Object>>() {});
    ...
    public static ResponseVS validateAccessRequest(File signedFile, Set<TrustAnchor> trustAnchors,
        ...
        User signer = cmsMessage.getSigner();
        try {
            CertUtils.verifyCertificate(trustAnchors, false, Arrays.asList(signer.getX509Certificate()));
        } catch(Exception ex) {
            log.log(Level.SEVERE, ex.getMessage(), ex);
        ...
        return new ResponseVS(ResponseVS.SC_OK).setCMS(cmsMessage);
    }
