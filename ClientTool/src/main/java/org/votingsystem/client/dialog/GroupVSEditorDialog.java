...
public class GroupVSEditorDialog extends DialogVS implements AddTagVSDialog.Listener {
    ...
    @FXML void initialize() {// This method is called by the FXMLLoader when initialization is complete
        ...
        tagVSButton.setGraphic(Utils.getIcon(FontAwesomeIcons.TIMES, Utils.COLOR_RED_DARK));
        tagVSButton.setOnAction(actionEvent -> {
            tagVS = null;
            tagVSButton.setVisible(false);
        });
        publishButton.setVisible(true);
        publishButton.setGraphic(Utils.getIcon(FontAwesomeIcons.CHECK, Utils.COLOR_RESULT_OK));
    ...
    private void loadOperationData(OperationVS operationVS) {
        ...
        switch (operationVS.getType()) {
            case CURRENCY_GROUP_EDIT:
                ProgressDialog.showDialog(new FetchGroupVSDataTask(operationVS.getMessage()),
                        ContextVS.getMessage("editGroupVSLbl"), getStage());
            ...
            case CURRENCY_GROUP_NEW:
                ProgressDialog.showDialog(new FetchGroupVSDataTask(operationVS.getNif()),
                        ContextVS.getMessage("newGroupVSLbl"), getStage());
    ...
    private void submitForm(){
        try {
            if(caption.getText() == null || "".equals(caption.getText().trim())) {
                showMessage(ResponseVS.SC_ERROR, ContextVS.getMessage("enterGroupVSNameMsg"));
                return;
            }
