...
public class SignatureBuilder {
    ...
    private static ConcurrentHashMap<String, SignatureBuilder> signatureServices	= new ConcurrentHashMap<>();
    ...
    public SignatureBuilder(KeyStore keyStore, String keyAlias, String password) throws Exception {
        init(keyStore, keyAlias, password);
    }

    public SignatureBuilder(MockDNIe mockDNIe) throws Exception {
    ...
    public CMSSignedMessage signDataWithTimeStamp(byte[] contentToSign) throws Exception {
        ...
        ResponseDto responseDto = HttpConn.getInstance().doPostRequest(timeStampRequest.getEncoded(),
                MediaType.TIMESTAMP_QUERY, org.votingsystem.Constants.TIMESTAMP_SERVICE_URL);
