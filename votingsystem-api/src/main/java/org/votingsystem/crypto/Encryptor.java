...
public class Encryptor {
    ...
    public Encryptor(X509Certificate localCert, PrivateKey localPrivateKey) {
        ...
        recipient = new JceKeyTransEnvelopedRecipient(localPrivateKey).setProvider(Constants.PROVIDER);
    }
    ...
    public byte[] decryptCMS(byte[] pemEncrypted) throws Exception {
        ...
        if (it.hasNext()) {
            ...
            CMSTypedStream recData = recipient.getContentStream(
                    new JceKeyTransEnvelopedRecipient(privateKey).setProvider(Constants.PROVIDER));
