...
public class DNIeSessionHelper {
    ...
    private static Logger logger = Logger.getLogger(DNIeSessionHelper.class);    
    ...
    public static Session getSession (char[] password, Mechanism signatureMechanism) throws Exception {
        ...
        try {
            ...
            if(slot == null) throw new VotingSystemException (
                    ContextVS.INSTANCE.getString("smartCardReaderErrorMsg"));
            token = slot.getToken();
            if(token == null) throw new VotingSystemException (
                    ContextVS.INSTANCE.getString("missingPKCS11ErrorMsg"));
            ...
            FileInputStream fis =  new FileInputStream(
                    VotingToolContext.DEFAULTS.APPDIR + ContextVS.CERT_RAIZ_PATH);
            ...
            while ((tokenCertificateObjects = pkcs11Session.findObjects(1)).length > 0) {
                ...
                if (CERT_SIGN.equals(cert.getLabel().toString())) {
                    certificadoUsuario = (X509Certificate)CertUtil.loadCertificateFromBytes(value);
                    ContextVS.INSTANCE.setSessionUser(
                            ContextVS.INSTANCE.getUserVS(certificadoUsuario));
                } else if (CERT_CA.equals(cert.getLabel().toString())) {
                    certificadoIntermedio = (X509Certificate)CertUtil.loadCertificateFromBytes(value);
                }
            }
            pkcs11Session.findObjectsFinal();
        } catch (Exception ex) {
            ...
            if ("CKR_PIN_INCORRECT".equals(ex.getMessage())) { 
                throw new VotingSystemException(
                        ContextVS.INSTANCE.getString("MENSAJE_ERROR_PASSWORD"));
            }
