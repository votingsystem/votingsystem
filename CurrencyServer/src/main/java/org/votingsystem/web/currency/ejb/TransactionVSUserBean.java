 ...
 */
@Stateless
public class TransactionVSUserBean {

    private static Logger log = Logger.getLogger(TransactionVSUserBean.class.getName());
    ...
    public ResultListDto<TransactionVSDto> processTransactionVS(TransactionVSDto request, TagVS tagVS) throws Exception {
        ...
        TransactionVS transactionVS = dao.persist(TransactionVS.USER(request.getSigner(), request.getReceptor(),
        ...
        resultListDto.setMessage(messages.get("transactionVSFromUserOKMsg", request.getAmount() +
    ...
    public TransactionVSDto validateRequest(TransactionVSDto dto) throws ValidationExceptionVS {
        if(TypeVS.FROM_USER != dto.getOperation()) throw new ValidationExceptionVS(
