...
<dom-module name="transaction-form">
<template>
    <style></style>
    <div style=" margin: 0 auto; max-width: 550px;">
        ...
        <div id="formDataDiv" style="padding: 0px 20px 0px 20px; height: 100%;">
            ...
            <div class="horizontal layout center center-justified">
                <input type="text" id="transactionSubject" class="form-control" required
                ...
                <div style="margin:0px 10px 0px 0px; padding:5px;">
                    <div hidden="{{tagSelected}}" class="layout horizontal center center-justified" style="font-size: 0.8em;">
                        ...
                        <div hidden="{{selectedTags.length == 0}}" style="margin: 0 0 0 10px;">
                            ${msg.transactionWithTagAdvertMsg}
                        </div>
...
<script>
    Polymer({
        ...
        submit: function () {
            ...
            if(!this.$.transactionSubject.validity.valid) {
                //this.$.transactionSubject.classList.add("formFieldError")
                this.$.transactionSubject.style.background = '#f6ccd0'
                alert("${msg.emptyFieldMsg}", "${msg.errorLbl}")
                return
            }
            var tagList = []
            ...
            operationVS.serviceURL = vs.contextURL + "/rest/transaction"
            operationVS.signedMessageSubject = "${msg.transactionFromGroupMsgSubject}"
            var signedContent = {operation:this.operation, subject:this.$.transactionSubject.value,
        ...
        transactionResponse:function(appMessageJSON) {
            ...
            if(ResponseVS.SC_OK == appMessageJSON.statusCode) {
                caption = "${msg.transactionOKLbl}"
            } else caption = '${msg.transactionERRORLbl}'
            alert(appMessageJSON.message, caption)
        },
        transactionFromUser:function(tagList) {
            var operationVS = new OperationVS(this.operation)
            operationVS.serviceURL = vs.contextURL + "/rest/transaction"
            operationVS.signedMessageSubject = "${msg.transactionFromUser}"
            var signedContent = {operation:this.operation, subject:this.$.transactionSubject.value,
                timeLimited:this.$.timeLimitedButton.checked, tags:tagList, amount: this.$.amount.value,
                currencyCode:this.$.currencySelector.getSelected(), toUserName:this.toUserName, toUserIBAN: this.getToUserIBAN()}
            operationVS.jsonStr = JSON.stringify(signedContent)
            operationVS.setCallback(this.transactionCallback)
            VotingSystemClient.setMessage(operationVS);
        },
        transactionCallback: function (appMessage) {
        ...
        init:function(operation, userName, userIBAN, userId) {
            ...
            switch(operation) {
                case Operation.FROM_GROUP_TO_MEMBER_GROUP:
                    this.operationMsg = "${msg.transactionFromGroupToMemberGroup}"
                ...
                case Operation.FROM_GROUP_TO_ALL_MEMBERS:
                    this.isWithUserSelector = false
                    this.operationMsg = "${msg.transactionFromGroupToAllMembers}"
                    this.selectReceptorMsg = '${msg.transactionToAllGroupMembersMsg}'
                    break;
                case Operation.FROM_USER:
                    this.operationMsg = "${msg.transactionFromUser} ${msg.forLbl} '" +
                            userName + "'"
                    this.fromUserName = null
                    this.fromUserIBAN = null
