...
<dom-module name="user-subscription-card">
    <template>
        <style>
        .userCard {
            width: 250px;
        ...
        .username {color: #888; margin: 3px 3px 5px 3px; white-space: nowrap; overflow: hidden;
        ...
        <div class="userCard" on-click="userSelected">
            <div hidden="{{nifHidden}}" class="nifColumn">{{subscription.user.nif}}</div>
            <!--<div style="width:200px;">{{subscription.user.iban}}</div>-->
            <div class="horizontal layout center-justified username flex">{{subscription.user.name}}</div>
    ...
    <script>
        Polymer({
            ...
            userSelected: function(e) {
                console.log(this.tagName + " - userSelected - userId: " + this.subscription.user.id)
                document.querySelector("#voting_system_page").dispatchEvent(new CustomEvent('user-selected', {detail:this.subscription.user.id}))
            }
        });
    </script>
</dom-module>
