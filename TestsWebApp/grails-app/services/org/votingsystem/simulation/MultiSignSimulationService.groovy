...
class MultiSignSimulationService {
    ...
    private void initServer() {
        ...
        ServerInitializer accessControlInitializer = new ServerInitializer(simulationData.getServerURL(), null);
	...
	private void sendSignatures(ActorVS actorVS){
        ...
        try {
            simulatorExecutor.execute(new Runnable() {
                @Override public void run() { sendSignatureRequests(actorVS); }
            });
            simulatorExecutor.execute(new Runnable() {
	...
	public void sendSignatureRequests (ActorVS actorVS) throws Exception {
        ...
        while(simulationData.getNumRequests() < simulationData.getNumRequestsProjected()) {
            if((simulationData.getNumRequests() - simulationData.
                ...
                signCompletionService.submit(new MultiSignTestSender(nifFrom, actorVS.getMultiSignedMessageTestServiceURL()));
        ...
        try {
            switch(statusFromResponse.getStatus()) {
                ...
                case Status.INIT_SERVER:
                    if(ResponseVS.SC_OK == statusFromResponse.getStatusCode()) {
                        sendSignatures(statusFromResponse.data);
                    } else finishSimulation(statusFromResponse);
                    break;
