            ...
            if(exception instanceof NotFoundException) {
                ...
                if(exception.getCause() instanceof ExceptionVS) {
                    ...
                    return Response.status(Response.Status.BAD_REQUEST).entity(exceptionVS.getMessage())
                        ...
                        "WebApplicationException: " + exception.getMessage()).type(javax.ws.rs.core.MediaType.TEXT_PLAIN).build();
                ...
                if(((ExceptionVS) exception).getMessageDto() != null) {
                    ...
                    return Response.status(messageDto.getStatusCode()).entity(
                            JSON.getMapper().writeValueAsBytes(messageDto)).type(MediaType.JSON).build();
                } else return Response.status(Response.Status.BAD_REQUEST).entity(exception.getMessage())
                        .type(javax.ws.rs.core.MediaType.TEXT_PLAIN).build();
            } else {
                log.log(Level.SEVERE, exception.getMessage(), exception);
                return Response.status(Response.Status.BAD_REQUEST).entity(
                        "Exception: " + exception.getMessage()).type(javax.ws.rs.core.MediaType.TEXT_PLAIN).build();
            }
        } catch (Exception ex) {
            log.log(Level.SEVERE, exception.getMessage(), exception);
            return Response.status(Response.Status.BAD_REQUEST).entity(
                    "Exception: " + ex.getMessage()).type(javax.ws.rs.core.MediaType.TEXT_PLAIN).build();
        }
    }
