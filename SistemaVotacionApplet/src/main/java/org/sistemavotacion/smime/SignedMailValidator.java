...
public class SignedMailValidator {
    ...
    private static Logger logger = LoggerFactory.getLogger(SignedMailValidator.class);
    ...
    public SignedMailValidator.ValidationResult verify() throws Exception {
        // iterate over all signatures and print results
        Iterator it = getSignerInformationStore().getSigners().iterator();
        Locale loc = Locale.ENGLISH;
        //only one signer supposed!!!
        SignedMailValidator.ValidationResult result = null;
        while (it.hasNext()) {
            SignerInformation signer = (SignerInformation) it.next();
            result = getValidationResult(signer);
