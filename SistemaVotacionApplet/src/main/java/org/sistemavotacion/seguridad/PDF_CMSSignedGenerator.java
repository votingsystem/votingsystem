...
public class PDF_CMSSignedGenerator extends CMSSignedGenerator 
    ...
    private static Logger logger = LoggerFactory.getLogger(PDF_CMSSignedGenerator.class);
...
//        }
    ...
    public CMSSignedData genSignedData(byte[] signatureHash, 
        ...
        ASN1ObjectIdentifier contentTypeOID = new ASN1ObjectIdentifier(
                CMSSignedGenerator.DATA);
        Map parameters = getBaseParameters(contentTypeOID, 
                new AlgorithmIdentifier(new DERObjectIdentifier(
                pdfDigestObjectIdentifier), new DERNull()), hash);
        AttributeTable attributeTable = sAttr.getAttributes(
