...
public class ArchivoFirmadoPanel extends JPanel implements ItemListener  {
    ...
    private static Logger logger = Logger.getLogger(ArchivoFirmadoPanel.class);
        ...
        try {
            if (signedFile.isValidSignature()) {
                resultadoFirmaButton.setText("<html><b>" + 
                        ContextVS.INSTANCE.getString("signatureOKLabel") + "</b><html>");
            resultadoFirmaButton.setIcon(new ImageIcon(getClass().
                    getResource("/resources/images/signature-ok_16x16.png")));

            } else {
                resultadoFirmaButton.setText("<html><b>" + 
                        ContextVS.INSTANCE.getString("signatureERRORLabel") + "</b><html>");
                resultadoFirmaButton.setIcon(new ImageIcon(getClass().
    ...
    private void resultadoFirmaButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_resultadoFirmaButtonActionPerformed
        try {
            ...
            firmantesDialog.mostrarInformacion(signedFile.getSMIMEMessageWraper());
        } catch (Exception ex) {
            logger.error(ex.getMessage(), ex);
        }
