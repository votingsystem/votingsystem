...
public class TimeStampCertFormDialog extends javax.swing.JDialog {
    ...
    private static Logger logger = Logger.getLogger(TimeStampCertFormDialog.class);
    ...
    private void validationButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_validationButtonActionPerformed
        ...
        try {
            ...
            setMessage(false, ContextVS.INSTANCE.getString("pemCertsErrorMsg"));
        }
        for(X509Certificate cert:certs) {
            ...
            try {
                timeStampToken.validate(new JcaSimpleSignerInfoVerifierBuilder().
                                setProvider(ContextVS.PROVIDER).build(cert));
                validationOK = true;
                validationCert = cert;
            } catch (Exception ex) {
                logger.error(ex.getMessage(), ex);
        ...
        if(!validationOK) {
            ...
            setMessage(true, ContextVS.INSTANCE.getString("pemCertsValidationOKMsg", 
