...
public class VoteResourceEJB {
    ...
    @POST @Path("/")
    public Response vote(@Context HttpServletRequest req, @Context HttpServletResponse res) throws Exception {
        byte[] voteSigned = FileUtils.getBytesFromStream(req.getInputStream());
        VoteDto voteDto = null;
        try {
            voteDto = XML.getMapper().readValue(voteSigned, VoteDto.class);
        } catch (Exception ex) {
            throw new IllegalArgumentException("Vote with bad formatt");
        }
        ...
        byte[] receipt = signatureService.signXAdES(voteSigned);
        return Response.ok().entity(receipt).build();
    }

    @POST @Path("/repository")
    public Response repository(@Context HttpServletRequest req, @Context HttpServletResponse res) throws Exception {
        ...
        List<Vote> votes = em.createQuery("select v from Vote v where v.certificate.revocationHashBase64 =:revocationHashBase64 and " +
                "v.state=:state").setParameter("revocationHashBase64", revocationHashBase64).setParameter(
