...
class BalanceService {
    ...
    public initWeekPeriod(Calendar requestDate) {
        ...
        int numTotalUsers = UserVS.createCriteria().count {
            or {
    ...
    public boolean initUserVSWeekPeriod(UserVS userVS, DateUtils.TimePeriod timePeriod, String transactionMsgSubject)
        ...
        for(String currency: currencyMap.keySet()) {
            for(Map.Entry<String, BigDecimal> tagVSEntry:  currencyMap[currency].entrySet()) {
                ...
                if(numInitPeriodTransaction > 0) throw new ExceptionVS("REPEATED COOIN_INIT_PERIOD TransactionVS for " +
                ...
                log.debug("$methodName - $currency - ${currentTagVS.getName()} - result: " +
                        "${reportFiles.getTagReceiptFile(tagVSEntry.getKey())}")
